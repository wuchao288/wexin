namespace Hidistro.SqlDal.Members
{
    ...
    public class MemberDao
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO aspnet_Members(GradeId,ReferralUserId,UserName,CreateDate,OrderNumber,Expenditure,Points,TopRegionId, RegionId,OpenId, SessionId, SessionEndTime,Password) VALUES(@GradeId,@ReferralUserId,@UserName,@CreateDate,0,0,0,0,0,@OpenId, @SessionId, @SessionEndTime,@Password)");
        ...
        public DbQueryResult GetMembers(MemberQuery query)
        {
            ...
            if (!string.IsNullOrEmpty(query.OpenId)) {
                ...
                builder.AppendFormat("OpenId = '{0}'", query.OpenId);
            }
        ...
        Label_0621:
            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "aspnet_Members m", "UserId", (builder.Length > 0) ? builder.ToString() : null, "*, (SELECT Name FROM aspnet_MemberGrades WHERE GradeId = m.GradeId) AS GradeName, (SELECT RealName FROM dbo.aspnet_Members b WHERE UserId = m.ReferralUserId) AS ReferralUserName,(select COUNT(1)  from aspnet_Distributors where UserId = m.UserId) IsOpen");
        }
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE aspnet_Members SET GradeId = @GradeId,OpenId = @OpenId,UserName = @UserName, RealName = @RealName, TopRegionId = @TopRegionId, RegionId = @RegionId,VipCardNumber = @VipCardNumber, VipCardDate = @VipCardDate, Email = @Email, CellPhone = @CellPhone, QQ = @QQ, Address = @Address, Expenditure = @Expenditure, OrderNumber = @OrderNumber,MicroSignal=@MicroSignal,ReferralUserId=@ReferralUserId WHERE UserId = @UserId");
            ...
            //更新店铺推荐人ID
            new DistributorsDao().UpdateReferralUserId(member.UserId, member.ReferralUserId);
        ...
        {
            string sql = "select count(1) from aspnet_Members";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(sql);
            object o = (this.database.ExecuteScalar(sqlStringCommand));
            if (o != null) {
