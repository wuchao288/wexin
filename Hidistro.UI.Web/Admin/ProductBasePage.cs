    ...
    public class ProductBasePage : AdminPage
    {
        ...
        private IList<string> GetOutsiteLinkImgs(string html)
        {
            ...
            for (int i = 0; i < matchs.Count; i++)
            {
                ...
                if ((item.ToLower(CultureInfo.InvariantCulture).IndexOf(masterSettings.SiteUrl.ToLower(CultureInfo.InvariantCulture)) == -1) && (item.ToLower(CultureInfo.InvariantCulture).IndexOf(masterSettings.SiteUrl.ToLower(CultureInfo.InvariantCulture)) == -1))
                {
                    list.Add(item);
                }
            }
